...
static inline handle_vec_t* layer_vec(server_t* s, int layer) {
...
static int32_t stack_find_index(const handle_vec_t* v, handle_t h) {
    if (!v) return -1;
    for (size_t i = 0; i < v->length; i++) {
        if (handle_vec_get(v, i) == h) return (int32_t)i;
    }
    return -1;
}

static void stack_update_indices(server_t* s, const handle_vec_t* v, size_t start) {
...
static void stack_vec_insert(server_t* s, handle_vec_t* v, size_t idx, handle_t h) {
    if (!v) return;
    if (idx > v->length) idx = v->length;
    handle_vec_push(v, 0);  // Placeholder
    if (idx + 1 < v->length) {
        memmove(&v->items[idx + 1], &v->items[idx], (v->length - 1 - idx) * sizeof(uint64_t));
    }
    v->items[idx] = (uint64_t)h;
    stack_update_indices(s, v, idx);
}

static bool stack_vec_remove(server_t* s, handle_vec_t* v, size_t idx) {
    if (!v || idx >= v->length) return false;
    if (idx + 1 < v->length) {
        memmove(&v->items[idx], &v->items[idx + 1], (v->length - idx - 1) * sizeof(uint64_t));
    }
    v->length--;
    if (idx < v->length) {
        stack_update_indices(s, v, idx);
...
static void debug_dump_layer(const server_t* s, layer_t l, const char* tag) {
    if (!s) return;
    const handle_vec_t* v = &s->layers[l];
...
void stack_remove(server_t* s, handle_t h) {
    ...
    handle_vec_t* v = layer_vec(s, layer);
    if (!v || v->length == 0) return;

    int32_t idx = c->stacking_index;
    if (idx < 0 || (size_t)idx >= v->length || handle_vec_get(v, idx) != h) {
...
void stack_place_above(server_t* s, handle_t h, handle_t sibling_h) {
    ...
    if (sib_idx < 0 || (size_t)sib_idx >= v->length || handle_vec_get(v, sib_idx) != sibling_h) {
...
static xcb_window_t find_window_below(server_t* s, client_hot_t* c) {
    ...
    if (v) {
        int32_t idx = c->stacking_index;
        if (idx < 0 || (size_t)idx >= v->length || handle_vec_get(v, idx) != c->self) {
            idx = stack_find_index(v, c->self);
        }
        if (idx > 0) {
            handle_t below_h = handle_vec_get(v, idx - 1);
    ...
    for (int l = layer - 1; l >= 0; l--) {
        handle_vec_t* lv = layer_vec(s, l);
        if (lv && lv->length > 0) {
            handle_t below_h = handle_vec_get(lv, lv->length - 1);
            client_hot_t* below = server_chot(s, below_h);
            if (below) return below->frame;
        }
    }
...
static xcb_window_t find_window_above(server_t* s, client_hot_t* c) {
    ...
    if (v) {
        ...
        if (idx >= 0 && (size_t)(idx + 1) < v->length) {
            handle_t above_h = handle_vec_get(v, idx + 1);
    ...
    for (int l = layer + 1; l < LAYER_COUNT; l++) {
        handle_vec_t* lv = layer_vec(s, l);
        if (lv && lv->length > 0) {
            handle_t above_h = handle_vec_get(lv, 0);
            client_hot_t* above = server_chot(s, above_h);
            if (above) return above->frame;
        }
    }
