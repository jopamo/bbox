...
// client_hot_t contains focus_node and fields touched by is_focusable + resize path
...
client_hot_t* server_chot(server_t* s, handle_t h) {
    // Very small lookup for tests: we stash pointers in (void*)conn for convenience
    // conn points to a NULL-terminated array of client_hot_t*
    client_hot_t** arr = (client_hot_t**)s->conn;
    if (!arr) return NULL;
    for (size_t i = 0; arr[i]; i++) {
        if (arr[i]->self == h) return arr[i];
    }
    return NULL;
}
...
void xcb_warp_pointer(void* conn, uint32_t src, uint32_t dst, int16_t src_x, int16_t src_y, uint16_t src_w,
...
static void test_wm_cycle_focus_selects_next_focusable(void) {
    ...
    client_hot_t b = make_client(200, 0, false, STATE_MAPPED, WINDOW_TYPE_DOCK);    // not focusable
