...
xcb_randr_query_version_reply_t* __wrap_xcb_randr_query_version_reply(xcb_connection_t* c,
                                                                      xcb_randr_query_version_cookie_t cookie,
                                                                      xcb_generic_error_t** e) {
    (void)c;
    (void)cookie;
    (void)e;
    if (fail_randr_reply) return NULL;
...
xcb_get_property_reply_t* __real_xcb_get_property_reply(xcb_connection_t* c, xcb_get_property_cookie_t cookie,
                                                        xcb_generic_error_t** e);

xcb_get_property_reply_t* __wrap_xcb_get_property_reply(xcb_connection_t* c, xcb_get_property_cookie_t cookie,
                                                        xcb_generic_error_t** e) {
    if (restore_active_window != XCB_NONE) {
        // We can't easily know WHICH property request this is without tracking cookies.
        // But for this test, we can cheat: if we set the flag, return the active window reply once.
        // Or better: xcb_stubs has a hook! But we are wrapping it.
        // Let's rely on the fact that server_init calls _NET_ACTIVE_WINDOW after _NET_CURRENT_DESKTOP.
        // This is brittle.
        // A better way: check if we are in server_init? No.
        ...
        // Let's just return what we want for ALL calls if set, assuming others handle garbage gracefully?
        // No, invalid property type might break things.

        // Let's implement a simple counter or just assume we can return a generic "valid" reply
        // and if it matches the expected type in the caller, it works.
        // server_init checks: if (r->type == XCB_ATOM_WINDOW && r->format == 32 && len >= 4)
        ...
        // The desktop check expects XCB_ATOM_CARDINAL. If we return WINDOW, it will ignore it. Perfect.
        return r;
    }
