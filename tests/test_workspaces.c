...
void test_workspace_switch_basics(void) {
    ...
    // Client 2: Desktop 1
    // Client 3: Desktop 0, but minimized (UNMAPPED state) - wait, if unmapped, we shouldn't map it.
    ...
    // c4 (Sticky): Should stay mapped (no action, or unmap/map?) -> Ideally no action.

    // Check server state
    assert(s.current_desktop == 1);

    // We can't easily check exact sequence without a log, but we can check last calls or counts.
    // Ideally we check if unmap was called for c1 and map for c2.

    // Let's assume the implementation optimizes and doesn't map already mapped windows?
    // But here c2 is "STATE_MAPPED" in our model, but physically it might be unmapped if we were on desktop 0.
    // Wait, in this test setup, we just created them. We didn't simulate previous state.
    // If we assume we started at Desktop 0:
    // c1 was visible. c2 was hidden.
    // So c1 should be unmapped. c2 should be mapped.
    ...
    // Let's implement wm_switch_workspace to be smart enough not to map STATE_UNMAPPED clients.
    ...
    // Last mapped could be C2 or C4 depending on iteration order (likely index order).
    // C2 is index 2, C4 is index 4. So C4 is last.
...
void test_client_move_to_workspace(void) {
    ...
    // Focus handling: if h1 was focused, it should lose focus if it moved to a hidden desktop
    s.focused_client = h1;
    // Move it back to 0
